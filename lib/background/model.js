...
model.getFeedById = function(db, feedId, callback) {
...
// TODO: this no longer works in version 8. Deprecating
// soon.
...
// TODO: deprecate this is refactor to update only.
// Insert or update
model.putFeed = function(db, feed, callback) {
  var store = db.transaction('feed','readwrite').objectStore('feed');
  
  // Some temp code for transition from database version 7 to version 8
  // as we never set it before. This will get called on the next polling
  // pass over the data so that I can properly test the new subscription
  // approach.
  //var uri = URI.parse(feed.url);
  //delete uri.scheme;
  // BUG: URI.toString is leaving in the leading slashes
  // so fix it. I really need to get around to fixing URI.
  //var temp = URI.toString(uri).substring(2);
  //feed.schemeless = temp;
  // Above commented out because it looks like it is done now.
