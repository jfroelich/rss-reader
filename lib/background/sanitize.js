...
handlers.a = function(anchor, baseURIObj) {
  if(!anchor.href) return UNWRAP;
  anchor.href = URI.resolve(baseURIObj, URI.parse(anchor.href));
  anchor.target = '_blank';
  return RETAIN;
};

handlers.embed = function(element) {
  if(element.src) {
    var src = URI.parse(element.src);
    if(src.scheme != 'https' && src.host && 
...
handlers.iframe = function(element, baseURI) {
  if(element.src) {
    var uri = URI.parse(element.src);
    if(!uri.scheme) {
      element.src = URI.resolve(baseURI, uri);
    }
  }
...
handlers.img = function(image, baseURIObj) {
  if(!image.src) return REMOVE;
  if(baseURIObj)
    image.src = URI.resolve(baseURIObj, URI.parse(image.src));
