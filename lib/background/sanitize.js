...
function evaluateRule(rule, node) {
  ...
  // localName is probably already lowercase, but just ensure we do 
  // a case insensitive comparison for now. This is probably wasted
  // processing. I should be lowercasing rule.tag when saving it, and I 
  // should know if the spec says localName is lowercase, or if chrome does 
  // some strange implicit equals ignore case behavior.
  var nodeName = node.localName.toLowerCase();
  var ruleTagName = rule.tag.toLowerCase();

  // localName is lowercase
  if(nodeName === ruleTagName) {
    var attrValue = node.getAttribute(rule.attr);
    if(attrValue) {
      if(rule.match) {
        // If the rule has a match, it should have a compiled regular 
        // expression.
        if(rule.re) {
          // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/test
          var ruleMatchedNodeResult = rule.re.test(attrValue);

          if(ruleMatchedNodeResult) {
            // NOTE: If we had observers for rule match logging, 
            // here is where we would notify them.
            // console.log('Rule matched: %s - %s', JSON.stringify(rule), node.outerHTML);  
          }

          return ruleMatchedNodeResult;
        } else {
          // console.warn('rule.re is undefined for rule %s', JSON.stringify(rule));
          // Fall through to return false
        }

      } else {
        // The node did not have the attribute, or the attribute was empty
        // Fall through to return false.
      }
    }
  }
...
function applyRules(node, rules) {
  ...
  return matched ? REMOVE : RETAIN;
}

// Returns a sanitized HTMLDocument object with the body element as the root'
// TODO: do I even need to return the object or is it modified in place because 
// pass by reference not pass by value?
// TODO: do I even need to pass in strBaseURL? Can we set it as a property in 
// doc in the caller, and use the URL object created from the doc parameter?
function sanitize(strBaseURL, doc, contentFilterRules) {
  ...
  walk(doc.body, function(node) {    
    if(node.nodeType == Node.COMMENT_NODE) {
      ...
      // Apply the rules, return REMOVE if any one matched
      var anyRuleMatchesResult = applyRules(node,contentFilterRules);
      if(anyRuleMatchesResult == REMOVE) {
        return REMOVE;
      }
      ...
      if(handler) {
        ...
        if(result == RETAIN || result == REPLACE) {
          ...
          return result;
        } else if(result == RETAIN_RAW) {
          return result;
        } else if(result == REMOVE) {
          return result;
        } else {
          console.warn('Undefined return behavior for result %s', result);
          return result;
        }
      } else {
        console.warn('No handler for node %s',node.outerHTML);
        return REMOVE;
      }
    }

    // It is not an element or a comment node, retain it. 
    // It is probably a TEXT_NODE

    return RETAIN;
  });

  // TODO: I don't think I need to do this
  // Return the modified doc object
  return doc;
}
