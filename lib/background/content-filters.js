...
contentFiltering.createRule = function(tag, attr, match) {
  ...
  extension.sendMessage({'type':'createContentFilter','rule':rule});
};

// Loads rules, removes the rule with the given id, then saves the rules,
// then notifies listeners.
// TODO: consider using splice instead of filter.
contentFiltering.removeRule = function(ruleId) {
  ...
  extension.sendMessage({type:'removedContentFilterRule',rule: ruleId});
};

// Returns a (sort of) user-friendly representation of a rule
