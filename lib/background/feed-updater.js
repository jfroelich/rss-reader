...
function updateFeed(db, feed, callback, timeout, contentFilterRules) {
  ...
  fetchFeed(feed.url, function(responseXML) {
    ...
    model.insertOrUpdateFeed(db, feed, function(feed) {
      ...
      each(fetchedFeed.entries, function(entry) {
        ...
        updateEntry(store, entry, onEntryUpdateSuccess, 
...
function updateEntry(store, entry, onSuccess, onError, contentFilterRules) {
  ...
  model.containsEntryHash(store, hash, function(hashedEntry) {
    ...
    if(entry.content) {
        ...
        var sanitizedDOM = sanitize(entry.feedLink, doc, contentFilterRules);
        if(sanitizedDOM) {
          // Trim the document
