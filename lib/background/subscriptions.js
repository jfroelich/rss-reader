 ...
 *
 * TODO: consider refactor as IEAF that produces only 3
 * public methods, uses strict, and does not use 
 * namespace within.
 *
 * TODO: I just realized, if I use an object i can maintain
 * state and dont have to pass around variables like I am doing.
 * I can define a private property in subscriptions called 
 * PutContext that I init when doing an add or an update. Just remember
 * to bind the prototype functions so the 'this' keyword works. Refactor
 * once I get this working again.
 */
var subscriptions = {};
...
subscriptions.add = function(params) {
  ...
  url = strings.stripControls((url || '').trim());
  ...
  if(db) {
    subscriptions.testURLExists(url,db,fetch,notify,oncomplete,onerror,timeout,cfrules);
  } else {
...
subscriptions.remove = function(feedId) {
  ...
  model.connect(function(db) {
    ...
    var keys = entries.index('feed').openKeyCursor(IDBKeyRange(feedId));
 ...
 */
subscriptions.testURLExists = function(url,db,fetch,notify,oncomplete,onerror,timeout,cfrules) {
  console.log('testURLExists %s', url);

  var schemeless = URI.parse(url);
  delete schemeless.scheme;
  schemeless = URI.toString(schemeless);
  console.log('schemeless %s', schemeless);

  db.transaction('feed').objectStore('feed').index('schemeless').get(schemeless).onsuccess = function(event) {
    if(this.value) {
      console.log('%s already exists in the database', url);
      onerror({type:'exists',url:url,existing:this.value.url});
    } else {
      console.log('%s does NOT already exist in the database', url);
      subscriptions.fetch(url,0,fetch,notify,oncomplete,onerror,timeout,cfrules);
    }
  };  
};
...
subscriptions.fetch = function(url,feedId,fetch,notify,oncomplete,onerror,timeout,cfrules) {
  ...
  if(feedId) {
    ...
    // Simulate a feed object with url property
    ...
    // pass along null for no fetchedFeed object
    model.connect(function(db) {
      subscriptions.insertFeed(url,db,null,notify,oncomplete,onerror,cfrules);  
    });
  }
};
...
subscriptions.insertFeed = function(db,fetchedFeed,notify,oncomplete,onerror,cfrules) {
  console.log('Inserting feed %s', url);
  
  //id, url, and possibly fetched are all preset on fetchedFeed here
  // already so they will appear in storableFeed as properties
...
subscriptions.addEntries = function(db,storedFeed,fetchedFeed,notify,oncomplete,onerror,cfrules,isNewSubscription) {
  ...
  collections.each(fetchedFeed.entries, function(fetchedEntry) {
    ...
    var getHashRequest = entryStore.index('hash').get(IDBKeyRange.only(hash));
    ...
    getHashRequest.onsuccess = function(event) {
      if(this.value) {
        ...
        console.log('An entry with hash code %s already exists, not inserting', hash);
        onUpdateError();
        return;
      }
      ...
      if(fetchedEntry.content) {
        var doc = htmlParser.parse(fetchedEntry.content);
        
        // Note: using feedLink as base url parameter to sanitize 
        // and not entry.link. I am still not entirely sure which 
        // one is correct.
        sanitizer.sanitize(storableEntry.feedLink, doc, cfrules);
        trimming.trimDocument(doc);
        storableEntry.content = doc.innerHTML;
      }
      
      // now insert. in this case reuse the transaction created 
      // when doing the hash equality test.
      
      var insertRequest = feedStore.add(storableEntry);
...
subscriptions.oncomplete = function(feed,oncomplete, notify, entriesProcessed,entriesAdded, isNewSubscription) {
  ...
  console.log('completed insert/update of id %s url %s processed %s added %s newSubscription? %s', 
    feed.id, feed.url, entriesProcessed, entriesAdded, isNewSubscription);
