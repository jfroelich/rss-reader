...
// TODO: create tests
// TODO: rename in-scope variables, remove bp prefix
...
function filterBoilerplate(documentObject) {
  ...
  const bestElement = bpFindHighScoreElement(documentObject);
  bpPrune(documentObject, bestElement);
}
...
function bpDeriveTextBias(element) {
  const text = element.textContent.trim();
  const textLength = 0.0 + text.length;
  const anchorLength = 0.0 + bpDeriveAnchorLength(element);
...
// undefined.
function bpDeriveAnchorLength(element) {
...
function bpDeriveAncestorBias(element) {
  let totalBias = 0;
  for(let childElement = element.firstElementChild; childElement;
    childElement = childElement.nextElementSibling) {

    const bias = bpAncestorBiases[childElement.localName];
...
function bpDeriveAttributeBias(element) {
  var totalBias = 0;
...
function bpFindHighScoreElement(documentObject) {
  ...
  for(var element of elementList) {
    ...
    var score = 0.0 + bpDeriveTextBias(element);
    ...
    score += bpDeriveAncestorBias(element);
    score += bpDeriveImageBias(element);
    score += bpDeriveAttributeBias(element);
...
function bpDeriveImageBias(parentElement) {
  let bias = 0.0;
  let imageList = bpFindChildImages(parentElement);
  for(let image of imageList) {
    bias += bpDeriveImageAreaBias(image) + bpDeriveImageTextBias(image);
  }
  bias += bpDeriveCarouselBias(imageList);
  return bias;
}
...
function bpFindChildImages(element) {
...
// Penalize carousels
function bpDeriveCarouselBias(imageList) {
  let bias = 0;
...
function bpDeriveImageTextBias(imageElement) {
  let bias = 0;
  ...
  if(bpFindImageCaption(imageElement)) {
    bias += 100;
  }
...
// Reward large images
function bpDeriveImageAreaBias(imageElement) {
...
function bpFindImageCaption(imageElement) {
...
function bpPrune(documentObject, bestElement) {
