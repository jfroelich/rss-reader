...
opt.onEnableNotificationsChange = function(event) {
  if(event.target.checked) chrome.permissions.request({permissions:['notifications']}, function() {});
  else chrome.permissions.remove({permissions:['notifications']}, function() {});
};

opt.onEnableBackgroundChange = function(event) {
  if(event.target.checked) chrome.permissions.request({permissions:['background']}, function() {});
  else chrome.permissions.remove({permissions:['background']}, function() {});
};
...
opt.init = function(event) {
  ...
  $('li#mi-rewriting').style.display = localStorage.URL_REWRITING_ENABLED ? 'block' : 'none';
