...
// because could conflate, use something like id=ruleX
...
opt.MessageDispatcher.onMessage = function(message) {
  if('createContentFilter' == message.type) {
    ...
    opt.onRewritingRuleCreated(message);
  }
  else if('displaySettingsChanged' == message.type) {
    // TODO: move code from stylize, decouple view/options message handling
...
opt.updateFeedCountMessage = function() {  
  ...
  $('#subscription-count').textContent = count ? ' ('+ count +')' : '';
};
...
opt.onFeedUnsubscribedMessage = function(event) {
  ...
  navigation.showSection($('#mi-add-subscription'));
...
opt.onFeedsImported = function(feedsImported, totalFeedsAttempted, elapsed) {
...
opt.onFileInputChanged = function(event) {
  ...
  reader.onload = opt.onFileReaderLoad;
...
opt.onFileReaderLoad = function(event) {
  var app = chrome.extension.getBackgroundPage();
  try {
  ...
  // TODO: this should be calling some other module, not 
  // opml-io
  app.opml.import(feeds, opt.onFeedsImported);
};
...
opt.onRewritingRuleRemoved = function(event) {
  var rule = event.rule;
  var element = $('ul#rewrite-rules-list li[rule="'+rule.id+'"]');
  element.parentNode.removeChild(element);
};

opt.onRewritingRuleCreated = function(event) {
  var rule = event.rule;
  var item = document.createElement('li');
  var app = chrome.extension.getBackgroundPage();
...
opt.initGeneralSettings = function(event) {
  
  var enableContentFilters = $('#enable-content-filters');
  enableContentFilters.checked = localStorage.ENABLE_CONTENT_FILTERS ? true : false;
  enableContentFilters.onchange = opt.onEnableContentFiltersChange;

  var enableURLRewriting = $('#rewriting-enable');
  enableURLRewriting.checked = localStorage.URL_REWRITING_ENABLED ? true : false;
  enableURLRewriting.onchange = opt.onEnableURLRewritingChange;

  $('#button-export-opml').onclick = opt.onExportOPMLClick;
  $('#button-import-opml').onclick = opt.onImportOPMLClick;
};

opt.initBackgroundMenu = function() {
...
opt.initHeaderFontMenu = function() {
...
opt.initDisplaySettings = function() {
  
  opt.initBackgroundMenu();
  opt.initHeaderFontMenu();
  opt.initBodyFontMenu();
  
  var inputChangedTimer, inputChangedDelay = 400;
  
  var entryBgcolor = $('#entry-background-color');
  ...
  var bodyFontSize = $('#body-font-size');
  ...
  justifyText.onchange = opt.onJustifyChange;
  
  var bodyLineHeight = $('#body-line-height');
...
opt.initContentFiltering = function(event) {
  var app = chrome.extension.getBackgroundPage();
...
opt.init = function(event) {
  ...
  opt.showSection($('li#mi-add-subscription'));
  ...
  app.collections.each($$('li.navigation-item'), opt.setNavigationOnClick);

  // Show a list of feeds in the navigation
  var feedCount = 0;
  app.model.connect(function(db) {
    app.model.forEachFeed(db, function(feed) {
      feedCount++;
  ...
  opt.initGeneralSettings();
  opt.initDisplaySettings();
  opt.initContentFiltering();
  opt.initURLRewriting();
