...
opt.startSubscription = function(url) {
  ...
  params.onerror = function(err) {
    console.log('subscription error %s', err);

    opt.SubscriptionMonitor.hide(function(){
      // TODO: show an error message.
      // if error.type == 'invalid url'
  ...
  params.contentFilterRules = app.contentFiltering.loadRules();
...
opt.onSubscribeMessage = function(message) {
  ...
  opt.SubscriptionMonitor.update('Successfully subscribed! Added '+ entriesAdded + 
    ' articles.');
  opt.SubscriptionMonitor.hide(function(){}, true);
...
opt.onFeedUnsubscribedMessage = function(event) {
  ...
  if(item) {
    item.removeEventListener('click', opt.feedListItemClick);
