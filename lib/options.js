...
opt.MessageDispatcher.onMessage = function(message) {
  if('displaySettingsChanged' == message.type) {
    ...
    var item = document.getElementById('feedlist li[feed="'+message.feed+'"]')
    if(item) {
    ...
    opt.updateFeedCountMessage();
    opt.showSection(document.getElementById('mi-subscriptions'));
...
opt.updateFeedCountMessage = function() {
  ...
  if(count) {
    // Show him da klamps!
    if(count > 1000) {
      document.getElementById('subscription-count').textContent = ' (999+)';
    } else {
      document.getElementById('subscription-count').textContent = ' ('+ count +')';
    }
...
opt.appendFeed = function(feed, insertedSort) {
  ...
  if(insertedSort) {
    var currentItems = document.getElementById('feedlist').childNodes;
    var added = false;
    ...
    for(var i = 0, len = currentItems.length; i < len; i++) {
      ...
      if(indexedDB.cmp(feed.title || '', currentKey || '') == -1) {
        added = true;
        document.getElementById('feedlist').insertBefore(item, currentItems[i]);
        break;
      }
    }
...
opt.hideSubscriptionPreview = function() {
  ...
  //document.getElementById('subscription-preview-load-progress').style.display = 'none';
  //document.getElementById('subscription-preview-continue').style.display = 'none';
};
...
opt.populateFeedDetailsSection = function(feedId) {
  model.connect(function(db) {
    db.transaction('feed').objectStore('feed').get(feedId).onsuccess = function(event) {
      ...
      document.getElementById('details-favicon').setAttribute('src', reader.fetch.getFavIconURL(feed.url));
...
opt.onFeedListItemClick = function(event) {
  ...
  opt.showSection(document.getElementById('mi-feed-details'));
  window.scrollTo(0,0);
};
...
opt.onSubscribeSubmit = function(event) {
  ...
  if(document.getElementById('discover-in-progress').style.display == 'block') return false;
...
opt.onExportOPMLClick = function(event) {
  backup.exportHTMLString(function(xmlString) {
...
opt.onCreateContentFilterClick = function(event) {
  ...
  document.getElementById('create-filter-attribute-name').value = '';
  document.getElementById('create-filter-attribute-value-match').value = '';
  opt.appendContentFilterRule(document.getElementById('content-filters-list'), rule);
};
...
opt.onContentFiltersPreviewSelectFeed = function(event) {
  document.getElementById('raw-select-entry').innerHTML = '';
  document.getElementById('raw-content-holder-hidden').innerHTML = '';
  document.getElementById('raw-content-viewer').innerHTML = '';
  document.getElementById('filtered-content-viewer').innerHTML = '';
  if(!event.target.value) return;
  var option = document.createElement('option');
  option.value = '';
  option.textContent = 'Select an article';
  document.getElementById('raw-select-entry').appendChild(option);
  var counter = 0;

  subscriptions.request(event.target.value, function(xml) {
    var feed = xml2json.transform(xml);
    Array.prototype.forEach.call(feed.entries, function(entry) {
      var option = document.createElement('option');
      ...
      document.getElementById('raw-content-holder-hidden').appendChild(li);
      counter++;
    }, function() {
...
opt.onTestRewritingRulesClick = function(event) {
  var inputValue = (document.getElementById('rewriting-test-input').value || '').trim();
  if(!inputValue) return;
  var outputValue = rewriting.rewriteURL(rewriting.loadRules(), inputValue);
  document.getElementById('rewriting-test-output').value = outputValue ? outputValue : 'No match';
};

opt.onCreateRewritingRuleClick = function(event) {

  var path = (document.getElementById('rewriting-create-path').value || '').trim();
  var param = (document.getElementById('rewriting-create-parameter').value || '').trim();
  ...
  document.getElementById('rewriting-create-path').value = '';
  document.getElementById('rewriting-create-parameter').value = '';
  ...
  button.textContent = 'Remove';
  item.appendChild(button);
  document.getElementById('rewrite-rules-list').appendChild(item);
};
...
opt.init = function(event) {
  ...
  document.getElementById('enable-notifications').onclick =
    opt.onEnableNotificationsChange;
  ...
  document.getElementById('enable-background').onclick = opt.onEnableBackgroundChange;
  ...
  document.getElementById('enable-idle-check').onclick = opt.onEnableIdleCheckChange;
  ...
  Array.prototype.forEach.call(document.querySelectorAll('ul#navigation-menu li'),
    opt.setNavigationOnClick);
  ...
  document.getElementById('enable-subscription-preview').onchange = opt.onEnableSubscriptionPreviewChange;
  document.getElementById('details-unsubscribe').onclick = opt.onUnsubscribeButtonClicked;
  document.getElementById('subscription-form').onsubmit = opt.onSubscribeSubmit;
  document.getElementById('enable-content-filters').checked = !!localStorage.ENABLE_CONTENT_FILTERS;
  document.getElementById('enable-content-filters').onchange = opt.onEnableContentFiltersChange;
  document.getElementById('rewriting-enable').checked = !!localStorage.URL_REWRITING_ENABLED;
  document.getElementById('rewriting-enable').onchange = opt.onEnableURLRewritingChange;
  document.getElementById('button-export-opml').onclick = opt.onExportOPMLClick;
  document.getElementById('button-import-opml').onclick = opt.onImportOPMLClick;
  document.getElementById('subscription-preview-continue').onclick = opt.onPostPreviewSubscribeClick;
  ...
  document.getElementById('entry-background-image').onchange = opt.onBackgroundImageChange;
  ...
  document.getElementById('select_header_font').onchange = opt.onHeaderFontChange;
  document.getElementById('select_body_font').onchange = opt.onBodyFontChange;
  ...
  document.getElementById('justify-text').onchange = opt.onJustifyChange;
  ...
  document.getElementById('create-filter-action').onclick = opt.onCreateContentFilterClick;
