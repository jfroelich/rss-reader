...
opt.appendFeed = function(feed, insertedSort) {
  ...
  item.setAttribute('title', strings.stripTags(feed.description) || '');
  ...
  title.textContent = strings.truncate(feed.title,300) || 'Untitled';
  ...
  model.connect(function(db) {
    db.transaction('feed').objectStore('feed').get(feedId).onsuccess = function(event) {
      ...
      $('#details-feed-description').textContent = 
        strings.stripTags(feed.description) || 'No description';
...
opt.onDiscoverFeedsComplete = function(query, results) {
  ...
  collections.each(displayableResults, function(result) {
    var item = document.createElement('li');
    ...
    a.title = strings.stripTags(result.title);
    a.innerHTML = strings.truncate(result.title, 70);
    item.appendChild(a);
    ...
    snippetSpan.innerHTML = 
      strings.truncate(result.contentSnippet.replace('<br>',''), 400);
...
opt.onContentFiltersPreviewSelectFeed = function(event) {
  ...
  subscriptions.request(event.target.value, function(xml) {
    ...
    collections.each(feed.entries, function(entry) {
      var option = document.createElement('option');
      option.value = counter;
      option.textContent = strings.truncate(entry.title, 100);
      $('#raw-select-entry').appendChild(option);
...
opt.onContentFiltersPreviewSelectEntry = function(event) {
  ...
  $('#raw-content-viewer').innerHTML = item.innerHTML.replace(/&lt;/g,
    '<br>$&').replace(/&gt;/g,'$&<br>');
  var html = htmlParser.parse(item.textContent);
  sanitizer.sanitize('http://', html, contentFiltering.loadRules());
  // Escape
...
opt.onURLRewritingRuleCreatedMessage = function(event) {
  ...
  item.textContent = rewriting.ruleToString(event.rule);
...
opt.onCreateRewritingRuleClick = function(event) {
  ...
  rewriting.createRule(path, param);
...
opt.onEnableNotificationsChange = function(event) {
  if(event.target.checked) {
    chrome.permissions.request({permissions:['notifications']}, function(granted) {
      console.log('Granted notifications permission? %s', granted);
    });
  } else {
    chrome.permissions.remove({permissions:['notifications']}, function(removed) {
      console.log('Revoked notifications permission? %s', removed);
    }); 
  }
};
...
opt.onEnableBackgroundChange = function(event) {
  if(event.target.checked) {
    chrome.permissions.request({permissions:['background']}, function(granted) {
      console.log('Granted background permission? %s', granted);
    });
  } else {
    chrome.permissions.remove({permissions:['background']}, function(removed) {
      console.log('Revoked background permission? %s', removed);
    });
  }
};
...
opt.init = function(event) {
  ...
  // Set to checked if has permission
  ...
  collections.each($$('ul#navigation-menu li'), opt.setNavigationOnClick);
  ...
  model.connect(function(db) {
    model.forEachFeed(db, function(feed) {
      ...
      option.textContent = strings.truncate(feed.title, 30);
      $('#raw-browser-select-feed').appendChild(option);
    }, null, true);
  });
  ...
  rewriting.loadRules().forEach(function(rule) {
    var item = document.createElement('li');
