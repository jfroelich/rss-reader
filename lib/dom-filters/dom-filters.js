...
export async function compositeDocumentFilter(doc, options = {}) {
  ...
  frameFilter(doc, options.emptyFrameBodyMessage);
  ensureBodyElementFilter(doc);
  iframeFilter(doc);
  commentFilter(doc);
  visibilityFilter(doc, options.contrastMatte, options.contrastRatio);
  ...
  lonestarFilter(doc);
  imageDeadFilter(doc);
  await imageReachableFilter(doc, options.reachableImageFilterTimeout);
  await imageDimensionsFilter(doc, options.imageDimensionsFilterTimeout);
  ...
  containerFilter(doc);
  listFilter(doc);
  tableFilter(doc, options.tableScanMaxRows);
  emphasisFilter(doc, options.emphasisMaxLength);
...
export function attributeEmptyFilter(doc) {
  ...
  const booleanAttributeNames = [
    ...
    'draggable', 'enabled', 'formnovalidate', 'hidden', 'indeterminate', 'inert', 'ismap',
    ...
    'selected', 'sortable', 'spellcheck', 'translate', 'truespeed', 'typemustmatch', 'visible'
  ];
...
export function imageLazyFilter(doc) {
  const lazyAttributeNames = [
    ...
    'data-original-desktop', 'data-baseurl', 'data-flickity-lazyload', 'data-lazy', 'data-path',
    'data-image-src', 'data-original', 'data-adaptive-image', 'data-imgsrc', 'data-default-src',
    'data-hi-res-src'
  ];
    ...
    if (list.firstChild) {
      ...
      if (firstElement) {
        ...
        if (firstElement.nextElementSibling) {
          ...
          if (listItemElementNames.includes(firstElement.localName)) {
