...
async function process_image(image, timeout) {
  ...
  if (url.protocol !== 'data:'
      && exts.includes(getPathExtension(url.pathname))) {
    for (const pair of pairs) {
      const width = parseInt(url.searchParams.get(pairs.w), 10);
      const height = parseInt(url.searchParams.get(pairs.h), 10);
      if (width > 0 && height > 0) {
  ...
  try {
    const fetched_image = await fetchImageElement(url, timeout);
