...
function appendFeed(feed, insertedSort) {
  ...
  // For later insertion sort
  listItem.setAttribute('sort-key', feed.title);
  // For finding for removal when unsubscribing
  listItem.setAttribute('feed',feed.id);
  
  var unsubImage = document.createElement('img');
  unsubImage.src = '/img/unsubscribe.gif';
  unsubImage.value = feed.id;
  unsubImage.title = 'Unsubscribe from ' + (feed.title || 'Untitled');
  unsubImage.addEventListener('click', onUnsubscribeButtonClicked);
  
  listItem.appendChild(unsubImage);
...
function onUnsubscribeButtonClicked(event) {
  ...
  app.unsubscribe(parseInt(event.target.value));
}
