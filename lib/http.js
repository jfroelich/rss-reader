...
function fetchHTMLDocument(params) {
  ...
  function onHTMLDocumentLoad() {
    ...
    // Sweet, this is definitely the post-redirect url
...
function augmentImageData(doc, baseURL, oncomplete) {
  ...
  var loadableImages = Array.prototype.filter.call(allBodyImages, function(image) {
    var source = (image.getAttribute('src') || '').trim();
    return source && !image.width && !isDataURL(source);
  });
