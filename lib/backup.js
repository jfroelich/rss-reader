...
// TODO: this can go further and also create the blob since
// nothing needs the intermediate string
function exportOPMLString(onComplete) {

  openIndexedDB(function(db) {
    getAllFeeds(db, onGetAllFeeds);
    //getAllFeeds(db, toOPMLString.bind(null, onComplete));
  });

  //openIndexedDB(loadAllFeedsAsOPMLString.bind(null, onComplete));
...
/*
function loadAllFeedsAsOPMLString(onComplete, db) {
  getAllFeeds(db, toOPMLString.bind(null, onComplete));
}

function toOPMLString(onComplete, feeds) {
  var xmlDocument = createOPMLDocument(feeds,'subscriptions.xml');
  var serializer = new XMLSerializer();
  var str = serializer.serializeToString(xmlDocument);
  onComplete(str);
