...
async function fetchHTML(urlString, timeoutMillis) {
  ...
  // Intentionally not catching parse exceptions. Allow exceptions to become
  // the rejection of the implied promise of the async function.

  // TODO: actually if I recall this never throws, instead it embeds the error
  // as an element of the result? Maybe I should be checking for that instead
  // and throwing an error in that case. Assert documentObject is defined,
  // and assert documentObject.documentElement is defined, and assert
  // that documentElement has tagName html
