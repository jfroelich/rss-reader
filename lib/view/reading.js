...
reading.scanForRead = function() {
  ...
  reading.app.model.connect(function(db) {
    reading.app.model.markRead(db, ids, function() {
      reading.app.browserAction.updateBadge();
...
reading.onScroll = function(event) {
  var deltaY = (reading.lastPageYOffset || pageYOffset) - pageYOffset;
  reading.lastPageYOffset = pageYOffset;

  if(deltaY >= 0) {
    return;
  }

  reading.scheduleScan();
};
...
window.addEventListener('resize', reading.scheduleScan);
document.addEventListener('scroll', reading.onScroll);