...
appending.hasScrolled = 0;

// Callback when scrolling
appending.onScroll = function(event) {

  // Calculate the scroll amount
  var deltaY = (appending.lastPageYOffset || pageYOffset) - pageYOffset;
  appending.lastPageYOffset = pageYOffset;

  // Exit if we scrolled down and we have previously scrolled
  if(deltaY >= 0 && appending.hasScrolled) {
    return;
  }

  appending.hasScrolled = 1;

  // Every downward scroll resets the append timer. It may be undefined
  // right now but clearTimeout does not mind.
  clearTimeout(appending.timer_);

  if(appending.container.childElementCount) {

    // We scrolled far enough down that we should append
    // We schedule here instead of calling append because we 
    // want to throttle scroll events triggering the append.

    var appendThreshold = pageYOffset + innerHeight + 100;
    if(appending.container.lastChild.offsetTop < appendThreshold) {
      
      appending.timer_ = setTimeout(appending.append, appending.DELAY);
    }
  }
};
...
appending.onPollCompleted = function(event) {
  ...
  // If all the entries in the UI are read and the user is idle
  // then append some more.
...
// Bindings
document.addEventListener('scroll', appending.onScroll);
