...
function onViewMessage(message) {
  if('displaySettingsChanged' == message.type) {
    ...
    if(!countUnreadSlides()) {
      ...
      // would be inaccurate.
      var isFirst = !document.querySelector('#slideshow-container').firstChild;
    ...
    if(cursor) {
      if(notAdvanced && offset) {
        ...
        if(isFirst && counter == 0) {
          ...
          // in doc order.
          currentSlide = document.querySelector('#slideshow-container').firstChild;
          isFirst = false;
        }
...
function showNextSlide() {
  if(countUnreadSlides() < 2) {
    appendSlides(function() {
        var c = document.querySelector('#slideshow-container');
...
function countUnreadSlides() {
  // TODO: should document.querySelectorAll the slides
  var slideNodes = document.querySelector('#slideshow-container').childNodes;
