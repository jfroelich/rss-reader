...
slideshow.appendSlide = function(entry, isFirst) {
  ...
  // Note: calamine returns an element that is not a document
  // it could be doc.body, or could be some other element
  var newBody = calamine.transform(doc, slideshow.CALAMINE_OPTIONS);

  // Note: would appendChild/importNode/adoptNOde work better here?
  // Note: still need to consider we only want the children.
  // maybe we want a docfrag insead of an element since appending a frag
  // excludes the root.
  // Am I seriously serializing then deserializing instead of 
  // just moving an object's parent?
  // since its from a diff doc, and we dont care about the rpesence
  // of the node in the original doc anymore, use adoptNode, which 
  // avoids importNode(deep)'s unecessary cloning.
  // but wait, how do i get it to replace body?

//newChild = document.adoptNode(externalNode);
//replacedNode = parentNode.replaceChild(newChild, oldChild);

  // This is assuming calamine uses the new mutate original doc approach
  // and does not return something.
  // calamine.transform(doc);
  // var scrubbedDoc = document.adoptNode(doc);
  // ... resolve URLs ...
  // something like this. forgot above that 
  // i am not inserting into this document but rather
  // into a node
  // // slide.replaceChild(newBody, content);
  // still wrong. i dont want the body element
  // maybe calamine should return a document fragment
  // or something. but then, how do we 'adopt' it? 
  // do we just adopt each of its children? is 
  // documentfragment a node that can be adopted?
/*
Various other methods can take a document fragment as an argument 
(e.g., any Node interface methods such as Node.appendChild and Node.insertBefore), 
in which case the children of the fragment are appended or inserted, not the
 fragment itself.

So I assume we can 'adopt' a document fragment. So if calamine returned 
a doc frag instead of document (or nothing), it would be easy.
*/
  // content.appendChild(scrubbedDoc.body);
  ...
  if(baseURI) {
    util.each(doc.getElementsByTagName('img'), function(img) {
