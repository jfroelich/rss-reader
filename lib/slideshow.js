...
// TODO: Mark the slide read upfront when displaying it
...
slideshow.appendSlides = function(oncomplete, isFirst) {
  var counter = 0, limit = 10, offset = slideshow.countUnread(), notAdvanced = 1;
  model.connect(function(db) {
...
slideshow.appendSlide = function(entry, isFirst) {
  ...
  title.textContent = util.truncate(entry.title, 200) || 'Untitled';
  slide.appendChild(title);

  var content = document.createElement('span');
  content.setAttribute('class', 'entry-content');
  content.innerHTML = entry.content || 'No content';
  slide.appendChild(content);
  
  /* // Insert <h2>Page x</h2> into the content every so often
  var i = 0, pageCount = 0;
  util.toArray(content.childNodes).forEach(function(part) {
    if(i++ % 4 == 0 && i > 4) {
      //console.log(entry.title + ' page ' + i);
      
      var sep = document.createElement('h2');
      sep.textContent = 'Page ' + (++pageCount);
      part.parentNode.insertBefore(sep, part);
  ...
  feedTitle.textContent = (entry.author || 'Unknown author') + entryPubDate;
  ...
  //slideshow.paginateContent(content);
  
};

// TODO: look into CSS regions
// This doesn't work, just a first go
slideshow.paginateContent = function(content) {
  
  var it = document.createNodeIterator(content, NodeFilter.SHOW_ELEMENT);
  var numColumns = 2;

  var x = (content.offsetWidth / numColumns).toFixed(0);
  var y = document.documentElement.clientHeight - 700;
  var node = null;

  var pager = document.createElement('h2');
  pager.textContent = 'PAGE';

  while(node = it.nextNode()) {
    if(node.offsetLeft > x) {
      if(node.offsetTop > y) {
        console.log('Should break at %s', node.textContent);
        node.parentNode.insertBefore(pager, node);
        break;
      }
    }
  } 
...
slideshow.showNextSlide = function() {
  if(slideshow.countUnread() < 2) {
    slideshow.appendSlides(function() {
        ...
        slideshow.actuallyShowNextSlide();
        slideshow.maybeShowNoUnreadArticlesSlide();
    }, false);
  } else {
    slideshow.actuallyShowNextSlide();
...
slideshow.actuallyShowNextSlide = function() {
...
slideshow.onKeyDown = function(event) {
  if(event.target != document.body) {
    console.log('not on body %s', event.target);
    return;
  }

  if(event.keyCode == '32' || event.keyCode == '39' || event.keyCode == '78') {
    if(event.keyCode == '32') {
  ...
  } else if(event.keyCode == '37' || event.keyCode == '80') {
    clearTimeout(slideshow.keyDownTimer);
