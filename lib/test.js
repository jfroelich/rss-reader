...
test.applyCalamine = function(url) {
  ...
  req.onload = function(event) {
    ...
    for(var i = bounds.start; i < bounds.end;i++) {
      ...
      if(block.image) {
        block.image.removeAttribute('width');
        block.image.removeAttribute('height');
        block.image.style.display = 'inline';
        block.image.style.float = 'left';
        block.image.style.padding = '10px';
        testContainer.appendChild(block.image);
        continue;
      }
      ...
      for(var b = 0; b < block.values.length;b++) {
        ...
        if(parent.localName == 'blockquote') {
          ...
          p.innerHTML += '<a href="'+ block.parentNodes[b].getAttribute('href') +'">' + block.values[b] + '</a> ';
