...
function transformDocument(doc) {
  ...
  // TODO: LI grouping into OLs is failing.
  segments.forEach(function(segment) {
    //console.log('%s', segment.localName);
  });
...
function mapBlocksToSegments(blocks) {
  ...
  // TODO: something is fishy about LIs not being logically grouped
  // into OLs here. If they are logically grouped then they should not 
  // get their own score.
...
function isListItemInAxis(node) {
  var root = node.ownerDocument.body;
  var parent = node.parentNode;
  while(root != parent && parent.localName != 'li') {
    parent = parent.parentNode;
  }
...
function findSegmentAncestor(node) {
  ...
  // LI grouping is failing here. Maybe because we are only looking
  // at immediate node if LI, not if path from node to body 
  // contains LI

  // Ok, it is failing. But there is a much larger problem with 
  // trying to do this. A given <li> could have an absolute shit
  // ton of the hierarchy in it, much like a div.
  // For example i see <ul><li><div>50% of the page</div></li></ul>
  // So logical grouping of list items is not quite right.

  // So this entire segments approach is flawed.
  // So really we just want to find the blocks and propagate scores
  // up. Then we want to propagate scores down for certain elements
  // (like penalize all items under a certain branch for being a branch
  // with a certain tag like nav)
  ...
  if(isListItem(node)) {
    ...
    return node;
  } else if(isListItemInAxis(node)) {
    console.log('%s > %s', parent, node.localName);
  } else {
  }
...
function getAnchorDensityBias(element) {
  ...
  if(element.wordCount) {
    ...
    // The problem is partly that we cap at some amount like 100
    // which is kind of arbitrary. It needs to be more 
    // relative to text length. It kind of overlaps with text density
    // but text density cares more about magnitude (which is good)
...
function getElementDensityBias(element) {
  ...
  // Oops, its charCount/cardinality.
