...
var SELECTOR_BLACKLIST = 'applet,base,basefont,button,command,datalist,'+
'dialog,embed,fieldset,frame,frameset,head,iframe,input,link,math,meta,'+
'noframes,object,option,optgroup,output,param,script,select,style,title,'+
'textarea',

/** Selector for elements that text scoring should avoid */
SELECTOR_NON_TEXT = 'applet,audio,canvas,iframe,embed,img,object,video',

/** Selector for elements that should not be considered empty */
SELECTOR_LEAF_LIKE = 'applet,audio,br,canvas,hr,img,iframe,object,embed,frame,video',
...
SELECTOR_UNWRAPPABLE = 'body,center,div,font,form,html,legend,span';
...
function transformDocument(doc, options) {
  ...
  each(doc.body.querySelectorAll(SELECTOR_BLACKLIST), removeNode);
  ...
  if(options.UNWRAP_UNWRAPPABLES) {
    each(doc.body.querySelectorAll('*'), function(element) {
      if(element && element != bestElement && (element.matches(SELECTOR_UNWRAPPABLE) || 
...
function scoreElement(element) {
  ...
  if(element.charCount && !element.matches(SELECTOR_NON_TEXT)) {
...
function isEmptyLikeElement(element) {
  return !element.firstChild && !element.matches(SELECTOR_LEAF_LIKE);
}
