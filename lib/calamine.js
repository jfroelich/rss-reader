...
function findBody(doc, blocks) {
  // TODO: In order to find the body content, aggregate scores 
  // on block above each parent (but never above body). Also, 
  // optionally consider weighting (e.g. only a portion of the score 
  // goes to the parent).
  blocks.forEach(function(block) {
  ...
  // Get all elements with a score attribute
 ...
 * are content.
 *
 * Award up to 30 points for low anchor density.
 */
