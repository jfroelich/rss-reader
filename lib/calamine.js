...
/** Whitespace is important for these elements */
/*var SEMANTIC_WHITESPACE_ELEMENT = {
  code:1, pre:1
};*/
...
function transformDocument(doc) {
  ...
  reduce(doc.body.getElementsByTagName('*'), 
    appendElementOrParentElementIfRemovable, []).forEach(removeNode);
  ...
  if(options.SHOW_BRANCH) {
    each(doc.body.getElementsByTagName('*'), setBranchAttribute);  
  }
...
function scoreElement(element) {
  ...
  if(element.charCount && elementCanContainText(element)) {
...
function elementCanContainText(element) {
  return !(element.localName in NON_TEXT_CONTAINER);
}
...
function setBranchAttribute(element) {
  if(element.branch) {
...
function trimAndMaybeRemoveTextNode(node) {
  ...
  if(INLINE_ELEMENT[node.previousElementSibling]) {
    if(!INLINE_ELEMENT[node.nextElementSibling]) {
      node.nodeValue = node.nodeValue.trimRight();
    }
  } else if(INLINE_ELEMENT[node.nextElementSibling]) {
...
/*
function inSemanticWhitespaceAxis(node) {
  var root = node.ownerDocument.body;
  var parent = node.parentElement;

  if(!root) {
    throw new Error('No body element found for inSemanticWhitespaceAxis');
  }

  while(parent && parent != root && !SEMANTIC_WHITESPACE_ELEMENT[parent.localName]) {
    parent = parent.parentElement;
  }

  // If parent != root, then our walk towards root found a semantic whitespace
  // element and stopped short of root.
  return parent != root;
}*/

function appendElementOrParentElementIfRemovable(removableElements, element) {
  if(element.childNodes.length || element.localName in LEAF_LIKE_ELEMENT) {
    return removableElements;
  }

  var parent = element, root = element.ownerDocument.body;
  while(parent != root && parent.childNodes.length == 1)
    parent = parent.parentElement;
  
  //console.log('Should remove %s', parent.outerHTML);
