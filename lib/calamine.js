...
function mapNodesToBlocks(nodes) {
  ...
  nodes.forEach(function(node) {
    ...
    // Note: all text nodes have parent nodes

    // TODO: <p><span><a></a></span></p>  Do I handle this case? 
    // Yes a is immediate parent of the node. But what about 
    // <p><a><b>text</b></a></p>? Look's like a bug?
    // We have to get the full path, the list of ancestors between the block ancestor
    // and the current node, and return true if any is anchor.
    if(isAnchor(node.parentNode)) {
      block.anchorWordCount = block.anchorWordCount ? block.anchorWordCount + 
...
function isMergeableBlockElement(node) {
  if(BLOCK_ELEMENT[node.localName]) {
    return true;
  } else {
    return false;
  }
