...
SELECTOR_UNWRAPPABLE = 'a:not([href]),blink,body,center,div,font,form,'+
'html,legend,span,tbody,thead',
...
ID_CLASS_BIAS = {
  ...
  head: -50, insta: -100, left: -75, license: -100,
  link: -100, logo: -50, main: 50,
...
function scoreElement(element) {
  ...
  if(element.matches('img')) {
    ...
    if(!isFinite(area)) {
      ...
      element.score += 200;
      if(element.parentElement && element.parentElement != element.ownerDocument.body) {
        element.parentElement.score = (element.parentElement.score || 0) + 200;
      }
    } else if(area > 50000) {
...
function filterElementAttributes(element) {
  // Filtering into a new array has the ancillary advance of 
  // cloning element.attributes which avoids the issue with removing
  // the live attributes node list while iterating.
  Array.prototype.filter.call(element.attributes, 
    isRemovableAttribute).forEach(removeAttribute);
}
...
function transformRuleElement(element) { 
  ...
  // TODO: this is not quite having the desired effect. It seems like
  // while the replacement occurs the <p></p> then gets stripped 
  // and then the sibling text node gets normalized into the preceding
  // text node. Therefore this ends up basically just removing
  // the element instead of moving it.
...
/**
 * We have to use the special ownerElement property 
 * for attribute nodes. parentNode is null because 
 * attributes are not supposed to be considered nodes 
 * due to spec changes.
 */
