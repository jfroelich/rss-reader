...
function calamineTransformDocument(doc, options) {
  ...
  if(options.FILTER_ATTRIBUTES) {
    forEach.call(body.getElementsByTagName('*'), function(element) {
      // attribute.ownerElement is deprecated :(
      Array.prototype.filter.call(element.attributes, function(attribute) {
  ...
  body.score = -Infinity;
  var bestElement = reduce.call(body.getElementsByTagName('*'), function(previous, current) {
...
function calaminePreprocessDocument(doc) {
  ...
  // Start by stripping comments. This is largely for compression but also because
  // if I ever re-introduce element cardinality as a metric I do not want to have
  // to mess with excluding comment nodes from the calculation.
  // NOTE: this step may be entirely unecessary
  eachNode(body, NodeFilter.SHOW_COMMENT, removeNode);

  // Remove all blacklisted elements and non-whitelisted elements
  // NOTE: I am NOT optimizing until I prove that this is slow. I am focusing
  // on accurate behavior and readable code and avoiding common mistakes
  // first. There are several obvious tempting optimizations but I am
  // resisting the urge.
  // I could move variables outside this function.
  // I could use getElementsByTagName instead of querySelectorAll.
  // I could do removes in reverse to avoid the check for undefined
  // that results from removing an element's parent while iterating forward
  // I could use a plain for or while loop.
  // I could merge the blacklist checking with the whitelist checking.
  // I could declare the callback functions once outside the function.
  // I could declare Array.prototype.forEach externally.
  // I could avoid building the strings by adding.

  // TODO: move iframe from blacklist to whitelist once supported

  // NOTE: an element should not appear in both these lists,
  // but these lists taken together should represent 'known'
  // elements.

  // Removable elements
  var SELECTOR_BLACKLIST = 'applet,base,basefont,button,'+
    'command,datalist,dialog,embed,fieldset,frame,frameset,'+
    'html,head,iframe,input,legend,link,math,'+
    'meta,noframes,object,option,optgroup,output,param,script,'+
  ...
  var SELECTOR_WHITELIST = 'a,abbr,acronym,address,area,'+
    ...
    'inset,label,li,kbd,main,mark,map,meter,nav,nobr,noscript,ol,'+
    'p,pre,progress,q,rp,rt,ruby,s,'+
    'samp,section,small,span,strike,strong,'+
    'st1,sub,summary,sup,vg,table,tbody,td,tfood,th,thead,time,'+
    'tr,track,tt,u,ul,var,video,wbr';

  // Remove elements in the blacklist
  var blacklistedElements = doc.body.querySelectorAll(SELECTOR_BLACKLIST);
  forEach.call(blacklistedElements, removeNode);
  ...
  Array.prototype.filter.call(allElements, function(element) {
    return !element.matches(SELECTOR_WHITELIST);
  }).forEach(removeNode);

  // Remove all images that do not have a source attribute
  forEach.call(body.querySelectorAll('img:not([src])'), removeNode);

  // Unwrap noscript elements before removing invisible elements. I would prefer
  // to remove noscript elements but I cannot figure out a better way to deal with
  // the template unhiding technique. One technique might be to specifically remove
  // noscripts that are not display:none, otherwise unwrap
  forEach.call(body.querySelectorAll('noscript'), unwrapElement);

  // Remove all invisible elements. We have to check whether the element is defined
  // each time during iteration because an ancestor of the element may have been removed
  // in a prior iteration, in the case of nested invisible elements, and also because of
  // iteration order, in that if this is called from getElementsByTagName, then in
  // document order, the parent element comes before the child element, meaning that
  // a hidden parent that is removed means the children should never be reached. But they
  // are reached, so we have to check.
  forEach.call(body.querySelectorAll('*'), function(element) {
    if(element && element.parentElement && isInvisibleElement(element)) {
      element.remove();
    }
  });

  // Remove one-dimensional images. These are always boilerplate
  // TODO: experiment with offsetWidth and offsetHeight?
  forEach.call(body.getElementsByTagName('img'), function(element) {
    if(!element) {
      return;
    }
  ...
  // Remove all the empty children and shove all the parents on the
  // stack
