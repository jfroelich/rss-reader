...
TODO: there is some kind of serious bug where this keeps fetching the same
articles.
  ...
  model.connect(function(db) {
    feed.entries.forEach(function(entry) {
      ...
      self.findEntryByLinkURL(db, entry.link, function(existingEntry) {
        if(existingEntry) {
          //console.log('%s already exists in the database, not fetching',existingEntry.link);
          entryProcessComplete();
        } else {
          //console.log('Fetching new URL %s', entry.link);
          fetchEntry(entry);
        }
      });
    });
  ...
  var fetchEntry = function(entry) {
    ...
    request.onload = function(event) {
      ...
      if(/text\/html/i.test(type)) {
        if(response.responseXML && response.responseXML.body) {
          entry.content = response.responseXML.body.innerHTML;
        } else {
          console.dir(response);
        }
      } else {
        entry.content = 'Unsupported content type error. The webpage at ' + response.responseURL + 
