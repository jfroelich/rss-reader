...
* This function is impure. The input object is modified. The channel gets a message that cannot be unsent. The database is modified.
* Some properties are automatically initialized. For creation these are properties
such as the initial read state and archive state. For update the dateUpdated property is automatically set to the call date.
...
* **DOMException** when a database error occurs, such as when using a bad id value, or creating a new entry that shares a url with an existing entry, or the database is closing at the time the transaction is started, or if the entry object is not serializable (*structured-cloneable*)
* **InvalidStateError** channel is closed when posting message

Beware the following caveat: in the event of a database error the database remains in the state prior to calling `write_entry` because the transaction never commits. However, in the event of a channel post error *the database is still modified*. In other words `write_entry` can fail with an error but still permanently modify the database. The input object will also still be modified.
...
### Return value
Returns a promise that resolves to the id of the stored entry object.
...
For created entries, this retrieves the new id generated by indexedDB.

### Implementation note on result of put in case of update
`IDBObjectStore.prototype.put` still returns the object's key path in the case of a put overwrite. In the case of create it returns the new value resulting from the auto-increment.
