...
Array.prototype.forEach.call(scorableElements, 
  calamine$scoreElement.bind(null, scores, annotate));

function calamine$scoreElement(scores, annotate, element) {
  var bias = calamine$getAttributeBias(element);
  if(!bias) return;
...
function calamine$getAttributeBias(element) {

  // NOTE: chrome is whining about compound let statements
  // even though there are none in this code, probably due to 
  // some buggy term rewriting, so using var
  // NOTE: we cannot yet use for..of over the token array 
  // because chrome de-opts

  var concatenatedValues = calamine$getAttributesAsString(element);

  // We have two tokenize strategies available. Right now tokenize2
  // seems to yield better performance tokenize1.  

  var tokenSet = calamine$tokenize2(concatenatedValues);
...
// Returns a subset of an element's attribute values as a 
// concatenated string. This is currently isolated to allow for 
// simpler performance testing.
...
// attributes
function calamine$getAttributesAsString(element) {
  return (element.id || '') + 
    (element.name || '') +
    (element.className || '') +
...
function calamine$tokenize1(string) {
...
function calamine$tokenize2(string) {
  var tokens = new Set();
  var token = [];
  var joined = '';
