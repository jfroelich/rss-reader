...
class FaviconService {
    ...
    function onConnect(connection) {
      if(connection) {
        ...
        if(document) {
          ...
          if(iconURL) {
            this.log.debug(
              'FaviconService: caching icon in prefetched document', url.href,
              iconURL.href);
        ...
        if(iconURL) {
          this.log.error('FaviconService: connection error, found icon in ' +
            'prefetched document', url.href, iconURL.href);
          callback(iconURL);
        } else {
  ...
  fetchDocument(context) {
    ...
    if('onLine' in navigator && !navigator.onLine) {
      ...
      // context.entry will be defined when he had a cache hit but the entry
      // was expired so we still tried to fetch. since we can't fetch, fallback
      // to using the last known association.
      if(context.entry) {
    ...
    function onFetchPage(event) {
      ...
      if(event.type === 'load') {
        ...
        if(document) {
          const inPageIconURL = this.findIconURLInDocument(document,
            responseURL);
          if(inPageIconURL) {
            ...
            if(this.cache && context.connection) {
              this.cache.addEntry(context.connection, context.url,
                inPageIconURL);
            ...
            this.log.debug('FaviconService: no icons in page',
              context.url.href);
            this.lookupPageResponseURL(context, new URL(responseURL));
          }
        } else {
          this.log.debug('FaviconService: undefined document',
            context.url.href);
          this.lookupPageResponseURL(context, new URL(responseURL));
        }
      } else {
  ...
  lookupPageResponseURL(context, pageResponseURL) {
  ...
  onRequestOrigin(context, pageResponseURL,
    event) {
    const originURL = new URL(context.url.origin);
