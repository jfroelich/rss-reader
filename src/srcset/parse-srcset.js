...
function parseSrcset(input) {
  ...
  var inputLength = input.length,
      ...
      regexTrailingCommas = /[,]+$/, regexNonNegativeInteger = /^\d+$/,
      ...
      url, descriptors, currentDescriptor, state, c,
    ...
    while (true) {
      ...
      if (state === 'in descriptor') {
        ...
        if (isSpace(c)) {
          ...
          // descriptor is not empty, append current descriptor to descriptors.
        ...
        if (isSpace(c)) {
          ...
          // Anything else
          // Set state to in descriptor. Set position to the previous character
          // in input.
        } else {
          state = 'in descriptor';
          pos -= 1;
  ...
  function parseDescriptors() {
    ...
    for (i = 0; i < descriptors.length; i++) {
      ...
      if (regexNonNegativeInteger.test(value) && (lastChar === 'w')) {
        ...
        // If the descriptor consists of a valid floating-point number followed
        ...
        // Apply the rules for parsing non-negative integers to the descriptor.
        // If the result is zero, let error be yes. Otherwise, let
    ...
    // whose URL is url, associated with a width width if not absent and a pixel
