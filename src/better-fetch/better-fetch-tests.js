...
import {AcceptError} from '/src/better-fetch/better-fetch.js';
...
export async function better_fetch_ordinary_test() {
  ...
  const path = '/src/better-fetch/better-fetch-test.html';
...
export async function better_fetch_local_404_test() {
  const path = '/src/better-fetch/this-file-does-not-exist.html';
  const url_string = chrome.extension.getURL(path);
  const url = new URL(url_string);
