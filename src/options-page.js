...
// slideshow page. Also, I want to focus on a better UI. I grew sick of the
// options page. For one, Google keeps changing how it works. Google changed the
// style of the options page too. Two, I like the idea of experimenting with a
// single page application (SPA). This is partly in preparation for toying with
// react or other view libraries. Three, it turns out it is pretty annoying to
...
// handle the situation of an element without an explicit opacity? My instinct
// is that this was a leftover setting from back when I was developing
// fade_element and was running into the unexpected situation of an element that
// did not have an opacity set, but I know that I've since rectified
// fade_element to handle that case, so I do not think this is needed. In fact I
// am relying on that behavior in several other places so this is rather
// inconsistent. Inconsistency is bad

// # TODO: section_show
// * if cannot find element to show, then what? while this is certainly
// indicative of a serious error, serious errors shouldn't happen at UI level,
// so something else should happen here?
