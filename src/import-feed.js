...
export async function importFeed(args) {
  ...
  // Fetch the feed
  const fetch_url = db.getURL(args.feed);
  const fetch_options = { timeout: args.fetchFeedTimeout };
  const response = await betterFetch(fetch_url, fetch_options);
  const response_url = new URL(response.url);

  // Check if redirected
  if (args.create && fetch_url.href !== response_url.href) {
    const existing_feed = await db.getResource(
      {
        conn: args.conn, mode: 'url', url: response_url, key_only: true,
      },
    );
    if (existing_feed) {
      const message = `Already subscribed to redirected feed url ${response_url.href}`;
      throw new db.errors.ConstraintError(message);
    }
  }

  // Possibly append the redirect url
  db.setURL(args.feed, response_url);

  const response_text = await response.text();
  const parsed_feed = feed_parser.parseFromString(response_text);
  ...
  if (args.create && args.iconn) {
    const icon_url = await lookupFeedFavicon(args.feed, args.iconn);
    if (icon_url) {
...
function import_entries(entries, args) {
  ...
  const promises = entries.map((entry) => {
    ...
    const emphasis_max_length = config.readInt('emphasis_max_length');
...
async function import_entry_noexcept(args) {
  let new_entry_id = 0;
  try {
    new_entry_id = await importEntry(args);
  } catch (error) {
