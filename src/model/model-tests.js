...
export async function activate_feed_test() {
  const db_name = 'model-activate-feed-test';
  ...
  // immediately attach it to the model because we want to ignore certain
  ...
  const id = await model.createFeed(feed);
  ...
  const stored_feed = await model.getFeed('id', id, false);
  ...
  try {
    await model.activateFeed(fake_feed_id);
  } catch (error) {
    activation_error = error;
  }
  assert(activation_error);
...
export async function deactivate_feed_test() {
  ...
  model.channel = channel;
  await model.deactivateFeed(feed_id, 'testing');
