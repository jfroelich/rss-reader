...
export async function activate_feed_test() {
  ...
  const id = await create_feed(model, feed);
  ...
  const stored_feed = await get_feed(model, 'id', id, false);
  ...
  try {
    await activate_feed(model, fake_feed_id);
  } catch (error) {
    activation_error = error;
  }
  assert(activation_error);
