...
// different services. For example it is used by both polling and subscribing.
// That's the problem. It should be broken down so that those two functions have
// no coupling. Those two have different concerns. For example, subscribing
// doesn't care about entries.

// TODO: https://www.sandimetz.com/blog/2016/1/20/the-wrong-abstraction

// This is a wrapper around the generic parse-feed.js that translates the
// generic parsed feed format into the app's specific storable feed format.

// TODO: just ran into the same impression again as the following todo, I hate
// the processEntries parameter. It should be two function calls or something.

// TODO: I do not love the processEntries parameter. While processing entries is
// surely related to post-fetch processing that needs to be done, I do not think
// it fits here. Rather than pass a flag, the caller should merely decide
// whether to call a separate function. Not calling the function is equivalent
...
// on the caller in these alternate scenarios. I want to minimize the knowledge
// and abstract away from the messiness of it. That is kind of what this
// function does right now. I just hate the boolean. At the same time this
// function has problems because it is mixing concerns. The fact that it
// requires response parameters like urls and file modification date are a
// signal that the abstraction is wrong. It is marrying the wrong things
// together. There are different stages to the pipeline of feed processing, and
// it feels like this is taking bits from stage 2 and 3 and 4 and calling it
// step 1.5. I think it would make sense to clearly enumerate the use cases,
// then revisit how well the abstraction responds to each case.
...
export default function coerceFeed(
  ...
  // objects.
  // TODO: this needs to be changed, rdb.js does not export feed magic, so I
  // need another way of doing this safely. Temporary hack just to keep it
  // working for now is to hardcode the magic. Completely unreliable.
  ...
  // Supply today's date as the date the feed was published if the feed did not
  // provide a date
  ...
  // TODO: setting the date the feed xml file was last modified is something
  // else's concern
  ...
  // Pull the entries property out of the parsed feed. The interal parser
  // includes the entries array as a part of the parsed feed, but the app's
  // storage format does not store entries per feed, it stores feeds and entries
  // separately, and the feeds it stores do not have an entries property. We
  // have to do this now, even if entries are not processed later, because
  // entries cannot be defined as a feed property.
  ...
  // TODO: I am not sure that filtering out duplicate entries should be a
  // concern of this function. In fact I think it shouldn't. This is another
  // instance of mixing together too many concerns. Dedup should be some kind of
  // explicit step in the feed processing pipeline. I am just not sure where.
  // And I don't like the amount of boilerplate it introduces because at some
  // point the caller will have so much responsibility and so many concerns to
  // take care of that the caller will probably be making mistakes. On the other
  // hand, pretty much every caller wants to make uses of this functionality at
  // this stage of the pipeline. All I would be doing is placing more burden on
  // the caller by making more pipeline steps explicit?
...
// If the entry has a link property, canonicalize and normalize it, baseURL is
