...
RefreshFeedIconsService.prototype._openDBOnSuccess = function(ctx, event) {
  ctx.db = event.target.result;
  this.getAllFeeds(ctx.db, this._onGetAllFeeds.bind(this, ctx));
};

RefreshFeedIconsService.prototype._openDBOnError = function(event) {
  this.log.error(event.target.error);
  this._onComplete(ctx);
};

RefreshFeedIconsService.prototype._onGetAllFeeds = function(ctx, feeds) {
...
RefreshFeedIconsService.prototype._lookup = function(ctx, feed) {
  ...
  const doc = null;
  this.iconService.lookup(lookupURL, doc, this._onLookup.bind(this, ctx, feed));
};

RefreshFeedIconsService.prototype._onLookup = function(ctx, feed, iconURL) {
...
RefreshFeedIconsService.prototype._onPutSuccess = function(ctx, feed, event) {
  this.log.debug('Updated feed', this.getFeedURL(feed));
};

// Treat database put errors as non-fatal
RefreshFeedIconsService.prototype._onPutError = function(ctx, feed, event) {
  this.log.error(event.target.error);
};

RefreshFeedIconsService.prototype._onComplete = function(ctx) {
  if(ctx.db) {
