...
import {dedup_entries} from '/src/operations/rdr-poll-feeds/dedup-entries.js';
import {filter_entry_content} from '/src/operations/rdr-poll-feeds/filter-entry-content.js';
...
PollService.prototype.init = async function(channel) {
  const promises = [rdr_create_conn(), rdr_create_icon_conn()];
...
PollService.prototype.fetch_entry = async function(entry) {
  ...
  if (url_is_augmentable(url)) {
    const response = await rdr_fetch_html(url, this.fetch_html_timeout);
    if (response.ok) {
      return response;
    }
  }
