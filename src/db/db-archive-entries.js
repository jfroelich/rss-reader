...
// ### Params
// * **max_age** {Number} in milliseconds, optional, defaults to two days, how
...
// * **InvalidStateError** if the channel is closed at the time messages are
// sent to the channel
//
// ### Return value
...
// performance sensitive, but it is still data-size sensitive. Entry objects can
// be rather large because of the content field. While using the cursor is
// slower and involves a ton of stack calls, it scales better. With the cursor,
// only a limited number of entry objects ever reside in memory at any one point
// in time.
...
// may cause an invalid state error if the channel is closed. This will cause
// the operation to reject with an error. However, by this point the transaction
// was committed. Therefore it is possible for this function to throw an error
// yet still permanently modify state.

// ### Implementation note regarding batch message posting
// This sends one message to the channel per entry archived, rather than sending
// a single message containing an array of archived entry ids. The number of
// archived entries can be very large. Sending very large messages is
// discouraged. This assumes that BroadcastChannels are better tuned for sending
// a large number of small messages than a small number of large messages.
...
// TODO: check that loaded entry is_entry? too paranoid?
