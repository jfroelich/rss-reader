...
// Notes:
// * Returns a promise that resolves when the operation has completed, the
// promise return value is an array of the deleted entry ids
// * The operation resolves once the internal database transaction has
// committed.
...
// immediately
// * This never actually checks if the feed exists, it just deletes whatever
// feed object corresponds to the given id, if it exists, because it just calls
...
// * If an error occurs when posting a message to the channel, the returned
// promise rejects with an error, but the transaction has still committed and
...
// * **reason_text** {String} optional, a categorical description of the reason
// for deletion, such as 'unsubscribe', or 'inactive'

// ### Errors
// * **TypeError** if the input feed id is invalid
// * **Rejection errors** database i/o errors as DOMException, InvalidStateError
// if channel is closed at time of call to postMessage

// ### Return value
// Returns a promise that resolves to an array of the entry ids that were
// deleted. The array is always defined but may be zero-length.
...
// count, so the badge text is out of date, so this refreshes the badge. Because
// the call to `refresh_badge` is un-awaited it will still be pending at time of
...
// * Should consider de-optimizing by removing the feed index from the entry
// store. Deletes are rare events. There is not a great need for this operation
// to be fast. Maintaining the feed index on the entry store is expensive. What
// else relies on the feed index? Rather than finding entries for the feed by
// querying the feed index this could simply walk the entire entry store, one
// entry at a time.
