...
class EntryStore {
  ...
  async removeAll(ids, chan) {
    const tx = this.conn.transaction('entry', 'readwrite');
    const proms = ids.map((id) => this.remove(tx, id, chan));
  ...
  async put(entry) {
    const tx = this.conn.transaction('entry', 'readwrite');
    return await this.putWithTx(tx, entry);
  }
  ...
  async putAll(entries) {
    const tx = this.conn.transaction('entry', 'readwrite');
    const proms = entries.map((entry) => this.putWithTx(tx, entry));
    return await Promise.all(proms);
  }
