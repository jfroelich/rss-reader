import * as platform from '/src/lib/platform.js';
import * as tls from '/src/lib/tls.js';

// Open the slideshow view in a tab.
export async function open_view() {
  // Check if the view is already open and switch to it
  const url_string = platform.get_extension_url_string('slideshow.html');
  const view_tab = await platform.find_tab(url_string);
  if (view_tab) {
    platform.update_tab(view_tab.id, {active: true});
    return;
  }

  // Otherwise, try and reuse the newtab tab
  const reuse_newtab = tls.read_boolean('reuse_newtab');
  if (reuse_newtab) {
    const newtab = await platform.find_tab('chrome://newtab/');
    if (newtab) {
      platform.update_tab(newtab.id, {active: true, url: url_string});
      return;
    }
  }

  // Otherwise, open the view in a new tab
  platform.create_tab({active: true, url: url_string});
}
