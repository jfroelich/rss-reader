...
function unwrapElementTest() {
  // Assert the typical case of a simple straightforward unwrap call completes as expected
  ...
  // Assert that calling unwrap on something other than an element throws an exception
  let unwrapNullError;
  try {
  ...
  // Assert that unwrapping an element that has no parent node does not trigger an exception and
  // leaves the document in its expected state (untouched)
  ...
  // Assert that when there is preceding text and not subsequent text, that only left space is
  // added.
  doc = parseHTML(
  ...
  // Assert that when there is no preceding text and there is subsequent text, that only right space
  // is added
  doc = parseHTML(
  ...
  // Assert that where there is both preceding text and subsequent text, that both left and right
  // space are added
  doc = parseHTML(
  ...
  // Assert that when there is nothing (neither text nodes or other nodes) within the element being
  // unwrapped, and there are adjacent text nodes on both sides, that one space is added between the
  // nodes to prevent merging.
  ...
  // Test a case to think more about. This works but I am not sure this is the desired behavior.
  doc = parseHTML(
...
function unwrapElementListTest() {
  ...
  // NOTE: the whitespace manipulation is wonky/imperfect/inexact. Since it is not very significant,
  // correctness is determined by non-whitespace, which we verify using a
  // compare-ignoring-whitespace approach.
