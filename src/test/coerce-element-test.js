...
// TODO: assert that p still exists to test that coerce does not affect elements it should not
// affect (surprise side effects)
// TODO: test that child nodes remain in the expected place, including both elements and text nodes
...
function coerceElementTest() {
  ...
  // Now replace the bs with cs. Assert that a second call does not somehow work unexpectedly due
  // to side effects. Basically assert there are no consequential side effects. In addition, test
  // that coerce works with fictional elements, as c is not a standard element.
