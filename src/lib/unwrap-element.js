...
export default function unwrapElement(element) {
  ...
  if (isTableElement || isListElement) {
    frag.append(' ');
  } else if (prev && prev.nodeType === TEXT && first && first.nodeType === TEXT) {
    frag.append(' ');
  }
  ...
  if (isTableElement) {
    for (let i = 0; i < element.rows.length; i += 1) {
      for (let j = 0, row = element.rows[i]; j < row.cells.length; j += 1) {
    ...
    while (item) {
      if (listItemElementNames.includes(item.localName)) {
  ...
  if (!first && prev && next && prev.nodeType === TEXT && next.nodeType === TEXT) {
    frag.append(' ');
  }
