import * as MimeUtils from '/src/mime/mime.js';

async function registerDWLinkFilterRule() {
  if (localStorage.DW_LINK_RULE_ID) return;
  const linkMatcher = new chrome.declarativeWebRequest.RequestMatcher(
      {resourceType: ['xmlhttprequest'], contentType: ['text/html']});
  const linkAction =
      new chrome.declarativeWebRequest.RemoveResponseHeader({name: 'link'});
  const linkRule = {conditions: [linkMatcher], actions: [linkAction]};
