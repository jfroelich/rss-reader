...
try {
  ...
  assert(false, 'Tokenizing undefined input did not throw an exception');
} catch (error) {
  // console.debug(error);
}
...
try {
  tokens = tokenizeHTML(input);
  assert(false, 'Tokenizing null input did not throw an exception');
} catch (error) {
  // console.debug(error);
}
...
console.log('TEST:', input);
tokens = tokenizeHTML(input);
assert(tokens.length === 2 && tokens[0] === 'a' && tokens[1] === '</b>', input);

// Test: text after single tag
input = '</b>a';
console.log('TEST:', input);
tokens = tokenizeHTML(input);
assert(tokens.length === 2 && tokens[0] === '</b>' && tokens[1] === 'a', input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<p>' && tokens[1] === 'a' &&
        tokens[2] === '</p>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<p  >' && tokens[1] === 'a', input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<p<a>' && tokens[1] === '>', input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === 'a' && tokens[1] === '<!---->', input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<!---->' && tokens[1] === 'b', input);
...
try {
  tokens = tokenizeHTML(input);
  assert(false, 'Tokenizing non-terminated comment did not throw error');
} catch (error) {
}
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<style>' && tokens[1] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<style>' && tokens[1] === ' ' &&
        tokens[2] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<style>' && tokens[1] === 'abc' &&
        tokens[2] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<style attribute>' &&
        tokens[1] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<style attribute=\'value\'>' &&
        tokens[1] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<style>' && tokens[1] === 'a/*b*/c' &&
        tokens[2] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<style>' &&
        tokens[1] === 'a/*\n\nb\n\n*/c' && tokens[2] === '</style>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<style>' &&
        tokens[1] === 'a/*</style>*/c' && tokens[2] === '</style>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<style>' &&
        tokens[1] === '<p>bad</p>' && tokens[2] === '</style>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<script>' &&
        tokens[1] === '</script>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<script>' && tokens[1] === ' ' &&
        tokens[2] === '</script>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 2 && tokens[0] === '<script type="text/javascript">' &&
        tokens[1] === '</script>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<script>' && tokens[1] === 'a//b\n' &&
        tokens[2] === '</script>',
    input);
...
tokens = tokenizeHTML(input);
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === 'a//b\r\n' && tokens[2] === '</script>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === '/*</script>*/' && tokens[2] === '</script>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === 'var a = \'b\';' && tokens[2] === '</script>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === 'if(a<b) console.log("c");' && tokens[2] === '</script>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === 'var a = `backquoted`;' && tokens[2] === '</script>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === 'var a = \'"as`df"\';' && tokens[2] === '</script>',
    input);
...
assert(
    tokens.length === 3 && tokens[0] === '<script>' &&
        tokens[1] === 'var a = /[a-z]/i;' && tokens[2] === '</script>',
    input);
